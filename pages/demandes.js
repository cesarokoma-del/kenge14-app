...
export default function Demandes() {
  ...
  async function refuserDemande(demande) {
    const motif = prompt('Motif du refus (visible par le candidat) :')
    if (!motif) return
    ...
    <Layout activePage="demandes">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <p className="text-sm text-blue-800">
          ...
          Si approuvée, le locataire est créé automatiquement et vous pouvez générer le contrat.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <button
          ...
          className={`rounded-2xl shadow-lg p-6 text-left transition ${
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-emerald-100">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">
          {filterStatut === 'toutes' ? 'Toutes les demandes' :
           filterStatut === 'en_attente' ? 'Demandes en attente' :
           filterStatut === 'approuvee' ? 'Demandes approuvées' : 'Demandes refusées'}
            ...
            {demandesFiltrees.map((d) => (
              <div
                ...
                className={`border rounded-xl p-4 ${
                  d.statut === 'en_attente' ? 'border-yellow-300 bg-yellow-50' :
                  d.statut === 'approuvee' ? 'border-emerald-300 bg-emerald-50' :
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    d.statut === 'en_attente' ? 'bg-yellow-200 text-yellow-900' :
                    d.statut === 'approuvee' ? 'bg-emerald-200 text-emerald-900' :
                ...
                <div className="space-y-1 text-sm text-gray-700">
                  ...
                  {d.date_debut_souhaitee && (
                    <p>📅 Souhaite emménager le {new Date(d.date_debut_souhaitee).toLocaleDateString('fr-FR')}</p>
                  )}
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Reçu le {new Date(d.date_demande).toLocaleDateString('fr-FR')}
                </p>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {d.statut === 'en_attente' && (
                    <>
                      <button
                        ...
                        className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2 rounded-lg text-sm font-semibold transition"
                      >
                        ✅ Approuver
                      </button>
                      <button
                  ...
                  {d.telephone && (
                    <a
                      href={`https://wa.me/${d.telephone.replace(/[^0-9]/g, '')}`}
                      target="_blank"
                      rel="noopener noreferrer"
