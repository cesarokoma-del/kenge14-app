...
// Renouvellements à venir (90 jours) - priorité signé > en_attente
export const getRenouvellements = async () => {
  ...
  // 🔑 Enrichissement avec PRIORITÉ MÉTIER : signé > en_attente > rien
  const enrichedData = data.map(contrat => {
    const renouvs = contrat.renouvellements || []
    
    // 🏆 RÈGLE D'OR : une signature obtenue est SACRÉE
    // Elle ne doit jamais être écrasée par une proposition envoyée par erreur
    ...
    if (signe) {
      ...
      // Sinon, le plus récent par created_at
