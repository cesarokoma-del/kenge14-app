...
export const getLocatairesEnRetard = async () => {
  ...
  // 2. Récupérer tous les contrats actifs avec locataire et appartement
  ...
  if (errContrats || !contrats) {
    return { data: [], error: errContrats }
  }

  // 3. Récupérer TOUS les paiements (on filtrera ensuite par contrat)
  const { data: paiements } = await supabase
    .from('paiements')
    .select('contrat_id, mois_concerne, statut')

  // 4. Le mois courant en nombre absolu
  const aujourdhui = new Date()
  const moisCourant = aujourdhui.getFullYear() * 12 + aujourdhui.getMonth()

  // 5. Pour chaque contrat, calculer le retard (logique identique à pages/paiements.js)
  const enRetard = contrats
    .map(contrat => {
      // Tous les paiements de ce contrat
      ...
      // Pas de paiement enregistré → on ne compte pas comme retard (nouveau contrat)
