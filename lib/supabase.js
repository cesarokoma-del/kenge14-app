...
// Renouvellements à venir (90 jours) - enrichi avec dernier renouvellement
export const getRenouvellements = async () => {
  ...
  // 🔑 Enrichissement : on calcule le "dernier renouvellement" pour chaque contrat
  const enrichedData = data.map(contrat => {
    const renouvs = contrat.renouvellements || []
    
    // Trier par created_at décroissant (le plus récent en premier)
    const sorted = [...renouvs].sort((a, b) => 
      new Date(b.created_at) - new Date(a.created_at)
    )
    
    const dernier = sorted[0] || null
    const dejaSigne = renouvs.some(r => r.statut === 'active')
