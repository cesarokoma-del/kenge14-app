...
// Parse une date ISO (YYYY-MM-DD) en Date locale à minuit pour éviter
// les décalages UTC qui peuvent faire reculer le mois d'un cran.
function parseDateLocale(input) {
...
export function calculerLoyersImpayes(contrat, dateFinISO, paiements) {
  ...
  const dateDebut = parseDateLocale(contrat.date_debut)
  const dateFin = parseDateLocale(dateFinISO)
