 ...
 */
export function genererDecompteFinPDF({ contrat, decompte, parametres, signatureBailleur, signatureLocataire }) {
  const doc = new jsPDF({
  ...
  const dateDebut = contrat.date_debut ? formatDate(contrat.date_debut) : 'N/A'
  const dateFinEffective = contrat.date_fin_effective ? formatDate(contrat.date_fin_effective) : formatDate(new Date())
  ...
  if (contrat.raison_fin) {
    doc.text(`Motif de fin : ${contrat.raison_fin.replace(/_/g, ' ')}`, 20, y + 18)
  }
  ...
  if (contrat.notes_fin) {
    ...
    const lignesNotes = doc.splitTextToSize(contrat.notes_fin, 180)
    doc.text(lignesNotes, 15, y)
  }

  // ============================================================
  // Pied de page : zones de signature
  // ============================================================
  const pageHeight = doc.internal.pageSize.height

  // Sur la dernière page, espace pour signatures
  doc.setPage(doc.internal.getNumberOfPages())

  let ySig = pageHeight - 45
  ...
  if (signatureBailleur) {
    // Image PNG centrée au-dessus de la ligne de signature
